...
class GradFlow(Callback):
    ...
    def __init__(self, check_nan=True, check_zero=True, check_explod=True, last_only=True, write_grad=False,
                 max_val=1e6, outfile=None, enforce_sanity=False, save_every=1):
        super().__init__()
        ...
        def make_graph():
            ...
            def build_graph(fn):
                if hasattr(fn, 'variable'):  # if GradAccumulator
                    ...
                    if cur_instance.check_zero and all(
                            cur_instance.is_zero(gi) for gi in fn_dict.get(fn, [None]) if gi is not None):
                        val.update(
                            fillcolor=cur_instance.NODE_COLOR["zero"], shape="hexagon")
                    else:
                        both_bad = 0
                        if cur_instance.check_nan and any(
                                cur_instance.is_nan(gi) for gi in fn_dict.get(fn, [None]) if gi is not None):
                            val.update(
                                fillcolor=cur_instance.NODE_COLOR["nan"])
                            both_bad += 1
                        if cur_instance.check_explod and any(
                                cur_instance.is_large(gi) for gi in fn_dict.get(fn, [None]) if gi is not None):
                            val.update(
...
class GradientInspector(Callback):
    ...
    def __init__(self, bottom_zoom=-0.001, top_zoom=0.02, max_norm=4, speed=0.05, log=False, update_at="backward",
                 figsize=(10, 6), showbias=False, outdir=None, clean_temps=False):
        super().__init__()
        ...
        self.outfile = os.path.join(self.outdir, 'summary_video.mp4')
        self.saving_n = 0
    ...
    def update(self, grads):
        ...
        self.fig.legend([Line2D([0], [0], color="#A3D6D2", lw=3),
                         Line2D([0], [0], color="#2F6790", lw=3), Line2D([0], [0], color="gray", lw=3)],
                        ['max-gradient', 'mean-gradient', "norm"], loc='upper right', bbox_to_anchor=(1, 1), fontsize=9)
