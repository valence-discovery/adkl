from torch.nn import Linear, Parameter, ReLU, Sequential, Sigmoid, Bilinear

from adkl.models.utils import *
...
class FusionNet(torch.nn.Module):
    def __init__(self, input_size, condition_size, nb_layers=1, residual=True):
        ...
        hidden_sizes = ([int(input_size / 2)] * nb_layers) if input_size > 1 else ([1] * nb_layers)
    ...
    def forward(self, phis, z):
        out_shape = phis.shape[:-1]
        if phis.dim() > z.dim():
            z = z.unsqueeze(1).expand(*out_shape, z.shape[-1]).reshape(-1, z.shape[-1])
            phis = phis.reshape(-1, phis.shape[-1])
        if phis.dim() != 2:
        ...
        return res.reshape(*out_shape, -1)
