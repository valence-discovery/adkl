...
class SequenceTransformer:
    ...
    def __init__(self, alphabet, one_hot=False, returnTensor=True):
        """
    ...
    def __transform_seq2intlist(self, seq):
        ...
        while i < len(seq):
            ...
            if self.alphabet2int.get(seq[i:i + 2]):
                ...
                result[l] = self.alphabet2int.get(seq[i], self.unknown_symbol)
                if result[l] == self.unknown_symbol:
                    warnings.warn(
                        'Symbol {} absent from the vocabulary. Consider checking our dataset or updating our vocabulary if necessary'.format(
                            seq[i]))
                i += 1
            l += 1
        return result[:l]

    def transform(self, sequences, return_one_hot=False):
        ...
        if self.one_hot:
            ...
            final_res = vectorized_sequences
...
class AdjGraphTransformer(MoleculeTransformer):
    ...
    def _transform(self, mol):
        ...
        return (totensor(normalize_adj(adj_matrix.astype(np.uint8)), gpu=False),
                totensor(atom_matrix.astype(np.float32), gpu=False))
